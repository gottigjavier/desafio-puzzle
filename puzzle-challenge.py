...
while remainder_not_repeated:
    ...
    remainder_as_list_item = [remainder]
    if str(remainder_as_list_item) in remainders_list_str or iteration_counter > max_iter_check: # Se evalúa el contador para evitar un posible ciclo infinito
        remainder_not_repeated = False
    else:
        remainders_list_str = addRemainderToList()
